...
ALGORITHMS = [
    'ERM',
    'IRM',
    "ELAS_ERM",   ###Custom algorithm
    #"ELAS_IRM",   ###Custom algorithm
    'Fish',
    'GroupDRO',
    'Mixup',
    'MLDG',
...
class ELAS_ERM(ERM):
    """
    Custom Algorithm: Elastic-Net-Like Regularizations on Representations
    """

    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ELAS_ERM, self).__init__(
                input_shape, 
                num_classes, 
                num_domains,
                hparams,
    ...
    @staticmethod
    def _elas_penalty(features, rho):
        device = "cuda" if features[0][0].is_cuda else "cpu"

        feat_dim = features.size(0)
        features = features.view(feat_dim, -1)
        _, s, _ = torch.svd(features)
        #s = torch.svd(features, compute_uv=False)

        ###Both norms are normalized by batch_size
        avg_nuclear_norm = torch.sum(s) / feat_dim
        avg_frobenius_norm = torch.linalg.norm(features, 'fro') / feat_dim

        result = rho * avg_nuclear_norm + (1 - rho) * avg_frobenius_norm
        return result

    def update(self, minibatches, unlabeled=None):
        device = "cuda" if minibatches[0][0].is_cuda else "cpu"
        elas_penalty_weight = (
                self.hparams["alpha"] if
                    self.update_count >= self.hparams["elas_penalty_anneal_iters"] else
                    0.0)
        ...
        elas_penalty = self._elas_penalty(all_features, self.hparams["rho"])
        ...
        for i, (x, y) in enumerate(minibatches):
            ...
            nll += F.cross_entropy(logits, y)
            #elas_penalty += self._elas_penalty(features, self.hparams["rho"])
        nll /= len(minibatches)
        #elas_penalty /= len(minibatches)
        loss = nll + (elas_penalty_weight * elas_penalty)

        if self.update_count == self.hparams["elas_penalty_anneal_iters"]:
            self.optimizer = torch.optim.Adam(
                    params=list(self.featurizer.parameters()) + list(self.classifier.parameters()),
                    lr=self.hparams["lr"],
                    weight_decay=self.hparams["weight_decay"])
        ...
        return {"loss": loss.item(),
                "nll": nll.item(),
                "elas_penalty": elas_penalty.item()}
...
class ELAS_IRM(ERM):
    ...
    def __init__(self, input_shape, num_classes, num_domains, hparams):
        super(ELAS_IRM, self).__init__(input_shape, num_classes, num_domains, hparams)
        self.optimizer = torch.optim.Adam(
                params=list(self.featurizer.parameters()) + list(self.classifier.parameters()),
                lr=self.hparams["lr"],
                weight_decay=self.hparams["weight_decay"]
    ...
    def _elas_penalty(features):
        ...
        result = self.hparams["rho"] * avg_nuclear_norm + (1 - self.hparams["rho"]) * avg_frobenius_norm
        return result

    def update(self, minibatches, unlabeled=None):
        device = "cuda" if minibatches[0][0].is_cuda else "cpu"
        irm_penalty_weight = (
                self.hparams["irm_lambda"] if
                    self.update_count >= self.hparams["irm_penalty_anneal_iters"] else
                    1.0)
        elas_penalty_weight = (
                self.hparams["alpha"] if
                    self.update_count >= self.hparams["elas_penalty_anneal_iters"] else
        ...
        for i, (x, y) in enumerate(minibatches):
            ...
            elas_penalty += self._elas_penalty(features)

        nll /= len(minibatches)
        irm_penalty /= len(minibatches)
        elas_penalty /= len(minibatches)

        loss = nll + irm_penalty_weight * irm_penalty + elas_penalty_weight * elas_penalty

        if self.update_count == self.hparams["irm_penalty_anneal_iters"] or self.update_count == self.hparams["elas_penalty_anneal_iters"]:
            self.optimizer = torch.optim.Adam(
